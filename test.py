...
for i in range(1, 232):
    print("Pressing button {0}".format(i))
    print("Sending report: NULL_CHAR*2+chr({i})+NULL_CHAR*5")
    write_report(NULL_CHAR*2+chr(i)+NULL_CHAR*5)
    # Release keys
    write_report(NULL_CHAR*8)
    # Press SHIFT + i
    print("chr(32)+NULL_CHAR+chr({i})+NULL_CHAR*5")
    write_report(chr(32)+NULL_CHAR+chr(i)+NULL_CHAR*5)
    print(datetime.now())
